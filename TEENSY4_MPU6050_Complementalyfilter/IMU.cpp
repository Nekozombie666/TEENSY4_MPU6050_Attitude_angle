...
void computeIMU () {
  ...
  for (axis = 0; axis < 3; axis++)
    gyroADCinter[axis] =  (int16_t)imu.gyroADC[axis];
...
// imu.gyroADC [axis]の長さは14ビット、スケール係数により、deltaGyroAngle16 [axis]の長さは14ビットのままです
  ...
  //Serial.print(scale);Serial.print(F(","));
  //Serial.print(deltaGyroAngle16[PITCH]);Serial.print(F(","));
  //Serial.print(deltaGyroAngle16[YAW]);
...
//これを行うには、EstVがすでにジャイロによって回転しているため、フィルターをスキップします
  ...
  //Serial.print(sqGX_sqGZ);Serial.print(F(","));
